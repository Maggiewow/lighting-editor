 ...
 * @LastEditors: 赵婷婷
 * @LastEditTime: 2021-11-19 15:59:35
 */
// 插件类，需要基于这个扩展
...
export default class Blockedit extends Plugin {
  init() {
    ...
    // 绑定一个事件 block
    this.editor.commands.add('broadcast', new command(this.editor));
  }

  // 具体参数看文档： https://ckeditor.com/docs/ckeditor5/latest/api/module_engine_model_schema-Schema.html
  _defineSchema() {
    const schema = this.editor.model.schema;
    // 创建一个模型，名字是 broadcastEdit
    schema.register('broadcastEdit', {
    ...
    schema.register('broadLabel', {
      isLimit: true,
      // 在哪个地方使用
      allowIn: 'broadcastEdit',
      // 设置改节点是块还是根，根可以回车
  ...
  _defineConverters() {
    ...
    // 定义转换器  broadcastEdit模型转换成视图  -> <div class="broadcastEdit"></div>
