 ...
 * @LastEditors: 赵婷婷
 * @LastEditTime: 2021-11-18 18:03:49
 */
// 插件类，需要基于这个扩展
...
export default class Blockedit extends Plugin {
  init() {
    ...
    // 绑定一个事件 block
    this.editor.commands.add('paralanguage', new command(this.editor));
  }

  // 具体参数看文档： https://ckeditor.com/docs/ckeditor5/latest/api/module_engine_model_schema-Schema.html
  _defineSchema() {
    const schema = this.editor.model.schema;
    // 创建一个模型，名字是 paralanguageEdit
    schema.register('paralanguageEdit', {
    ...
    schema.register('paraLabel', {
      isLimit: true,
      // 在哪个地方使用
      allowIn: 'paralanguageEdit',
      // 设置改节点是块还是根，根可以回车
  ...
  _defineConverters() {
    ...
    // 定义转换器  paralanguageEdit模型转换成视图  -> <div class="paralanguageEdit"></div>
    conversion.elementToElement({
      model: 'paralanguageEdit',
      view: {
        name: 'div',
        classes: 'labelLine',
      },
